...
export const ProviderComparison: React.FC = () => {
  return (
    <section className="py-32 bg-transparent text-zinc-900 relative overflow-hidden z-10">
      {/* Background Decor */}
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center space-y-6 mb-24 cursor-default">
          ...
          <motion.h3 
            ...
            className="text-4xl md:text-6xl font-black tracking-tighter text-zinc-900 max-w-4xl mx-auto leading-[1.1]"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
          {comparisonData.map((col, i) => (
            <motion.div
              ...
              className="group bg-white/40 backdrop-blur-md border border-zinc-200/50 rounded-[2.5rem] p-8 lg:p-10 transition-all duration-300 hover:bg-white/90 hover:shadow-2xl hover:shadow-zinc-200/50 hover:-translate-y-1"
            >
              <div className="flex items-center gap-5 mb-10 pb-8 border-b border-zinc-100">
                <div className="p-4 bg-white rounded-2xl shadow-sm border border-zinc-100 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500">
                  {col.icon}
                </div>
                <h4 className="text-xl lg:text-2xl font-black text-zinc-900 tracking-tight italic opacity-80 group-hover:opacity-100 transition-opacity">vs. {col.title}</h4>
              </div>
                ...
                {col.advantages.map((adv, j) => {
                  ...
                  return (
                    <li key={j} className="flex gap-4 items-start group/item">
                      ...
                      <span className="text-zinc-700 text-sm lg:text-base leading-relaxed font-medium group-hover/item:text-zinc-900 transition-colors">
                        {parts.length > 1 ? (
                          <>
